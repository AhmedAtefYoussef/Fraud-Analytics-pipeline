...
class FeatureEngineer:
    ...
    def __init__(
        self, target_encoding_smoothing: float = 10.0, kpca_components: int = 3
    ):
    ...
    def fit(self, X_train: pd.DataFrame, y_train: pd.Series) -> "FeatureEngineer":
        ...
        for train_idx, val_idx in skf.split(temp_df, y_train):
            ...
            smooth_map = (
                stats["sum"] + self.target_encoding_smoothing * self.global_target_mean
            ) / (stats["count"] + self.target_encoding_smoothing)

            # Map to fold_val
            oof_encoded.iloc[val_idx] = (
                fold_val["hour"].map(smooth_map).fillna(self.global_target_mean)
            )
        ...
        self.hour_target_map = (
            (
                global_stats["sum"]
                + self.target_encoding_smoothing * self.global_target_mean
            )
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.fitted:
            raise RuntimeError(
                "FeatureEngineer must be fit before transform can be called."
            )
        ...
        X_out["Hour_Target_Encoded"] = hours.map(self.hour_target_map).fillna(
...
def run_feature_engineering() -> None:
    ...
    engineer = FeatureEngineer(
        target_encoding_smoothing=config.get(
            "features.target_encoding_smoothing", 10.0
        ),
    ...
    if resampling_strategy == "smote_tomek":
        logger.info(
            f"Applying SMOTE-Tomek balancing to Train features. Initial target distribution: {y_train.value_counts().to_dict()}"
        )
        ...
        logger.info(
            f"Resampling complete. Balanced target distribution: {y_train_res.value_counts().to_dict()}"
        )
    else:
